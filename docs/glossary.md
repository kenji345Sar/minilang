...
同じ `env` という単語だが、Web 開発で出てくる**環境変数**（`PATH`、`DATABASE_URL`、`NODE_ENV` など）とは別物。

| | Web 開発の `env`（環境変数） | minilang の `env` |
|---|---|---|
| 何の対応表か | OS／プロセスが持つ設定値 | プログラム内の変数 |
| 例 | `PATH`、`DATABASE_URL`、`NODE_ENV=production` | `{"x": 10, "y": 40}` |
| 設定する場所 | シェル、`.env` ファイル、デプロイ設定 | プログラム内の代入文 `x = 10;` |
| 寿命 | プロセスの起動から終了まで | REPL / プログラムの実行中 |
| 誰が読む | プログラム全体（OS 経由） | Evaluator（インタプリタ内部） |

共通の根っこは英単語の **environment（環境）**。計算機の世界では「**名前と値の対応表（context）**」を指す一般的な用語で、Web 開発の方は OS の外側の実行環境、言語処理系の方はインタプリタ内部の変数テーブル、と適用範囲が違うだけ。

混乱を避けたいときは：

- Web 開発の方 → **環境変数**（environment variables）
- minilang の方 → **変数環境** または **シンボルテーブル**（symbol table）
...
文字列で書かれたプログラムを読み込み、**その場で解釈して実行する**言語処理系。Lexer / Parser で AST に変換した後、Evaluator がその AST を辿って実行する、という形が典型。

minilang はインタプリタ。Python、JavaScript、Ruby も同じ系統。

→ 詳細：[README の「minilang はインタプリタ」セクション](../README.md)

## コンパイラ（compiler）

文字列で書かれたプログラムを**別の形に変換するだけ**で、実行は別の仕組みに任せる言語処理系。変換先は機械語、バイトコード、別の高級言語などさまざま。

C、Rust、Go はコンパイラ系（ソースコード → 機械語 → CPU が実行）。minilang はコンパイラではない（変換と実行を Evaluator が一緒にやる）。

## AST（抽象構文木）

[AST / 構文木 / 抽象構文木](#ast--構文木--抽象構文木) を参照。インタプリタが「実行する単位」として持っているデータ構造。
